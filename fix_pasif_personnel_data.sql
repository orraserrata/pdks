-- Pasif personellerin verilerini korumak için düzenleme
...
SELECT 
    ...
    p.soyisim,
    p.aktif,
    COUNT(pgc.id) as duzenli_kayit_sayisi
FROM personel p
LEFT JOIN personel_giris_cikis_duzenli pgc ON p.kullanici_id = pgc.kullanici_id
WHERE p.isim ILIKE '%berfin%' OR p.soyisim ILIKE '%berfin%'
GROUP BY p.kullanici_id, p.isim, p.soyisim, p.aktif;

-- 2. Berfin'in ham verilerini kontrol et
SELECT 
    kullanici_id,
    giris_tarihi,
...
WHERE kullanici_id IN (
    SELECT kullanici_id FROM personel 
    WHERE isim ILIKE '%berfin%' OR soyisim ILIKE '%berfin%'
)
GROUP BY kullanici_id, giris_tarihi
ORDER BY giris_tarihi DESC
LIMIT 10;

-- 3. Pasif personellerin verilerini korumak için admin_locked=true yap
UPDATE personel_giris_cikis_duzenli 
SET admin_locked = true
WHERE kullanici_id IN (
    SELECT kullanici_id FROM personel WHERE aktif = false
);

-- 4. generate_attendance_pairs fonksiyonunu güncelle - sadece admin_locked=false olanları sil
CREATE OR REPLACE FUNCTION generate_attendance_pairs()
RETURNS void AS $$
BEGIN
    -- Önce sadece admin_locked=false olan düzenli kayıtları temizle
    DELETE FROM personel_giris_cikis_duzenli 
    WHERE admin_locked = false;
    
    -- Ham verileri giriş-çıkış çiftlerine dönüştür (sadece aktif personeller için)
    ...
    -- Tek kalan girişler (çıkışı olmayan) - sadece aktif personeller için
    INSERT INTO personel_giris_cikis_duzenli (
        kullanici_id, 
        giris_tarihi, 
        cikis_tarihi, 
...
-- 5. Berfin'in verilerini yeniden oluştur
